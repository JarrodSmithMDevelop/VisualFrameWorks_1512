...
NSString * Smith_Jarrod_Project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Smith_Jarrod_Project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_smith_jarrod_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Smith_Jarrod_Project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Smith_Jarrod_Project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_smith_jarrod_project1 = [[Smith_Jarrod_Project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_smith_jarrod_project1];
	
	NSString *_smith_jarrod_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _smith_jarrod_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_smith_jarrod_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Smith_Jarrod_Project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.smith_jarrod_project1.kroll" 
